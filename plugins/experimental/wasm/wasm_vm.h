...
struct WasmFuncTypeHelper<N, ReturnType, ContextType, ParamType, ReturnType(ContextType, Args...)> {
  using type =
    typename WasmFuncTypeHelper<N - 1, ReturnType, ContextType, ParamType, ReturnType(ContextType, Args..., ParamType)>::type;
};
...
#define FOR_ALL_WASM_VM_EXPORTS(_f)                                                                                       \
  _f(WasmCallVoid<0>) _f(WasmCallVoid<1>) _f(WasmCallVoid<2>) _f(WasmCallVoid<4>) _f(WasmCallVoid<5>) _f(WasmCallVoid<8>) \
    _f(WasmCallWord<0>) _f(WasmCallWord<1>) _f(WasmCallWord<3>)
...
#define FOR_ALL_WASM_VM_IMPORTS(_f)                                                                                           \
  _f(WasmCallbackVoid<0>) _f(WasmCallbackVoid<1>) _f(WasmCallbackVoid<2>) _f(WasmCallbackVoid<3>) _f(WasmCallbackVoid<4>)     \
    _f(WasmCallbackWord<0>) _f(WasmCallbackWord<1>) _f(WasmCallbackWord<2>) _f(WasmCallbackWord<3>) _f(WasmCallbackWord<4>)   \
      _f(WasmCallbackWord<5>) _f(WasmCallbackWord<6>) _f(WasmCallbackWord<7>) _f(WasmCallbackWord<8>) _f(WasmCallbackWord<9>) \
...
public:
   ...
   * @return one of WasmVmValues from well_known_names.h e.g. "wavm"
   */
  virtual std::string_view vm() PURE;
   ...
   * by the proxy process it is up to the user to ensure that the code is both safe and is built
   ...
   */
  virtual void link(std::string_view debug_name, bool needs_emscripten) PURE;
   ...
   */
  virtual void setMemoryLayout(uint64_t stack_base, uint64_t heap_base, uint64_t heap_base_pointer) PURE;
   ...
   * (getFunction).
   * @param vm_context a context which represents the caller: in this case the proxy itself.
   */
   ...
   */
  virtual std::optional<std::string_view> getMemory(uint64_t pointer, uint64_t size) PURE;
   ...
   * proxy). This function can be used to chase pointers in VM memory.
   ...
   */
  virtual void makeModule(std::string_view name) PURE;
   ...
   * is no such section.
   */
  virtual std::string_view getUserSection(std::string_view name) PURE;

  /**
   * Get typed function exported by the WASM module.
   */
#define _GET_FUNCTION(_T) virtual void getFunction(std::string_view function_name, _T *f) PURE;
...
#define _REGISTER_CALLBACK(_T)                                                                     \
  virtual void registerCallback(std::string_view moduleName, std::string_view function_name, _T f, \
                                typename ConvertFunctionTypeWordToUint32<_T>::type) PURE;
...
#undef _REGISTER_CALLBACK
   ...
   */
  virtual std::unique_ptr<Global<Word>> makeGlobal(std::string_view module_name, std::string_view name, Word initial_value) PURE;
   ...
   * @return a Global object which can be used to access the exported global.
   */
  virtual std::unique_ptr<Global<double>> makeGlobal(std::string_view module_name, std::string_view name,
                                                     double initial_value) PURE;
};
using WasmVmPtr = std::unique_ptr<WasmVm>;

// Exceptions for issues with the WasmVm.
struct WasmVmException : public std::exception {
  explicit WasmVmException(std::string msg) : msg_(msg) {}
  std::string msg_;
};

// Exceptions for issues with the WebAssembly code.
struct WasmException : public std::exception {
  explicit WasmException(std::string msg) : msg_(msg) {}
  std::string msg_;
};
...
// Create a new low-level WASM VM of the give type (e.g. "wavm").
WasmVmPtr createWasmVm(std::string_view vm);
