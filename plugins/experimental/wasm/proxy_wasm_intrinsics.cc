...
static Context *
ensureContext(uint32_t context_id, uint32_t root_context_id)
{
  if (!context_map) {
...
static RootContext *
ensureRootContext(uint32_t context_id, std::unique_ptr<WasmData> root_id)
{
  if (!root_context_map)
    root_context_map = new std::unordered_map<std::string, RootContext *>();
  if (!context_map) {
    context_map = new std::unordered_map<int32_t, std::unique_ptr<ContextBase>>();
  }
...
static ContextBase *
getContextBase(uint32_t context_id)
{
  if (!context_map)
    return nullptr;
  auto it = context_map->find(context_id);
...
Context *
getContext(uint32_t context_id)
{
  if (!context_map)
    return nullptr;
  auto it = context_map->find(context_id);
...
static RootContext *
getRootContext(uint32_t context_id)
{
  if (!context_map)
    return nullptr;
  auto it = context_map->find(context_id);
...
proxy_onStart(uint32_t root_context_id, uint32_t root_id_ptr, uint32_t root_id_size, uint32_t vm_configuration_ptr,
              uint32_t vm_configuration_size)
{
  ensureRootContext(root_context_id, std::make_unique<WasmData>(reinterpret_cast<char *>(root_id_ptr), root_id_size))
    ->onStart(std::make_unique<WasmData>(reinterpret_cast<char *>(vm_configuration_ptr), vm_configuration_size));
}

extern "C" EMSCRIPTEN_KEEPALIVE uint32_t
proxy_validateConfiguration(uint32_t root_context_id, uint32_t ptr, uint32_t size)
{
  return getRootContext(root_context_id)->validateConfiguration(std::make_unique<WasmData>(reinterpret_cast<char *>(ptr), size)) ?
           1 :
           0;
}

extern "C" EMSCRIPTEN_KEEPALIVE uint32_t
proxy_onConfigure(uint32_t root_context_id, uint32_t ptr, uint32_t size)
{
  return getRootContext(root_context_id)->onConfigure(std::make_unique<WasmData>(reinterpret_cast<char *>(ptr), size)) ? 1 : 0;
}

extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onTick(uint32_t root_context_id)
{
  getRootContext(root_context_id)->onTick();
}

extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onCreate(uint32_t context_id, uint32_t root_context_id)
{
  ensureContext(context_id, root_context_id)->onCreate();
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterHeadersStatus
proxy_onRequestHeaders(uint32_t context_id)
{
  return getContext(context_id)->onRequestHeaders();
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterMetadataStatus
proxy_onRequestMetadata(uint32_t context_id)
{
  return getContext(context_id)->onRequestMetadata();
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterDataStatus
proxy_onRequestBody(uint32_t context_id, uint32_t body_buffer_length, uint32_t end_of_stream)
{
  return getContext(context_id)->onRequestBody(static_cast<size_t>(body_buffer_length), end_of_stream != 0);
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterTrailersStatus
proxy_onRequestTrailers(uint32_t context_id)
{
  return getContext(context_id)->onRequestTrailers();
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterHeadersStatus
proxy_onResponseHeaders(uint32_t context_id)
{
  return getContext(context_id)->onResponseHeaders();
}

extern "C" EMSCRIPTEN_KEEPALIVE FilterMetadataStatus
proxy_onResponseMetadata(uint32_t context_id)
{
...
extern "C" EMSCRIPTEN_KEEPALIVE FilterTrailersStatus
proxy_onResponseTrailers(uint32_t context_id)
{
  return getContext(context_id)->onResponseTrailers();
}

extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onDone(uint32_t context_id)
{
  getContext(context_id)->onDone();
}

extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onLog(uint32_t context_id)
{
  getContext(context_id)->onLog();
}

extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onDelete(uint32_t context_id)
{
...
extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onHttpCallResponse(uint32_t context_id, uint32_t token, uint32_t header_pairs_ptr, uint32_t header_pairs_size,
                         uint32_t body_ptr, uint32_t body_size, uint32_t trailer_pairs_ptr, uint32_t trailer_pairs_size)
{
  getRootContext(context_id)
    ->onHttpCallResponse(token, std::make_unique<WasmData>(reinterpret_cast<char *>(header_pairs_ptr), header_pairs_size),
...
extern "C" EMSCRIPTEN_KEEPALIVE void
proxy_onQueueReady(uint32_t context_id, uint32_t token)
{
