...
inline std::vector<std::pair<StringView, StringView>>
WasmData::pairs()
{
...
template <typename Pairs>
void
exportPairs(const Pairs &pairs, const char **ptr, size_t *size_ptr)
{
  if (pairs.empty()) {
    *ptr      = nullptr;
    *size_ptr = 0;
...
struct Tuple3Hash {
  ...
  std::size_t
  operator()(const std::tuple<T, U, V> &x) const
  {
...
public:
  ...
  using HttpCallCallback       = std::function<void(std::unique_ptr<WasmData> header_pairs, std::unique_ptr<WasmData> body,
...
public:
  ...
  virtual void onHttpCallResponse(uint32_t token, std::unique_ptr<WasmData> header_pairs, std::unique_ptr<WasmData> body,
                                  std::unique_ptr<WasmData> trailer_pairs);
  ...
  bool httpCall(StringView uri, const HeaderStringPairs &request_headers, StringView request_body,
                const HeaderStringPairs &request_trailers, uint32_t timeout_milliseconds, HttpCallCallback callback);
...
public:
  ...
  virtual FilterMetadataStatus
  onRequestMetadata()
  {
    return FilterMetadataStatus::Continue;
  }
  virtual FilterDataStatus
  onRequestBody(size_t /* body_buffer_length */, bool /* end_of_stream */)
  {
    return FilterDataStatus::Continue;
  }
  virtual FilterTrailersStatus
  onRequestTrailers()
  {
  ...
  virtual FilterDataStatus
  onResponseBody(size_t /* body_buffer_length */, bool /* end_of_stream */)
  {
    return FilterDataStatus::Continue;
  }
  virtual FilterTrailersStatus
...
#define ROOT_FACTORY(_c) \
  [](uint32_t id, StringView root_id) -> std::unique_ptr<RootContext> { return std::make_unique<_c>(id, root_id); }
#define CONTEXT_FACTORY(_c) \
  [](uint32_t id, RootContext *root) -> std::unique_ptr<Context> { return std::make_unique<_c>(id, root); }
...
// Generic selector
inline Optional<WasmDataPtr>
getProperty(std::initializer_list<StringView> parts)
{
  size_t size = 0;
  for (auto part : parts) {
...
inline bool
getStringValue(std::initializer_list<StringView> parts, std::string *out)
{
...
inline WasmResult
getRequestProtocol(std::string *result)
{
...
template <typename T>
inline bool
getValue(std::initializer_list<StringView> parts, T *out)
{
  auto buf = getProperty(parts);
...
continueRequest()
{
  return proxy_continueRequest();
}
inline WasmResult
continueResponse()
{
  return proxy_continueResponse();
}
inline WasmResult
sendLocalResponse(uint32_t response_code, StringView response_code_details, StringView body,
                  const HeaderStringPairs &additional_response_headers, uint32_t grpc_status)
{
  const char *ptr = nullptr;
  size_t size     = 0;
  exportPairs(additional_response_headers, &ptr, &size);
  return proxy_sendLocalResponse(response_code, response_code_details.data(), response_code_details.size(), body.data(),
                                 body.size(), ptr, size, grpc_status);
}
inline WasmResult
sendLocalResponse(uint32_t response_code, StringView response_code_details, StringView body,
                  const HeaderStringPairs &additional_response_headers)
{
  ...
  exportPairs(additional_response_headers, &ptr, &size);
  return proxy_sendLocalResponse(response_code, response_code_details.data(), response_code_details.size(), body.data(),
                                 body.size(), ptr, size, 0);
}
inline void
clearRouteCache()
{
  proxy_clearRouteCache();
}

// SharedData
inline WasmResult
getSharedData(StringView key, WasmDataPtr *value, uint32_t *cas = nullptr)
{
  uint32_t dummy_cas;
  const char *value_ptr = nullptr;
  size_t value_size     = 0;
...
inline WasmResult
setSharedData(StringView key, StringView value, uint32_t cas = 0)
{
  return proxy_setSharedData(key.data(), key.size(), value.data(), value.size(), cas);
}

inline WasmDataPtr
getSharedDataValue(StringView key, uint32_t *cas = nullptr)
{
  WasmDataPtr data;
...
// SharedQueue
inline WasmResult
registerSharedQueue(StringView queue_name, uint32_t *token)
{
  return proxy_registerSharedQueue(queue_name.data(), queue_name.size(), token);
}

inline WasmResult
resolveSharedQueue(StringView vm_id, StringView queue_name, uint32_t *token)
{
  return proxy_resolveSharedQueue(vm_id.data(), vm_id.size(), queue_name.data(), queue_name.size(), token);
}

inline WasmResult
enqueueSharedQueue(uint32_t token, StringView data)
{
  return proxy_enqueueSharedQueue(token, data.data(), data.size());
}

inline WasmResult
dequeueSharedQueue(uint32_t token, WasmDataPtr *data)
{
...
// Headers/Trailers
inline WasmResult
addHeaderMapValue(HeaderMapType type, StringView key, StringView value)
{
  return proxy_addHeaderMapValue(type, key.data(), key.size(), value.data(), value.size());
}

inline WasmDataPtr
getHeaderMapValue(HeaderMapType type, StringView key)
{
  const char *value_ptr = nullptr;
  size_t value_size     = 0;
...
inline WasmResult
replaceHeaderMapValue(HeaderMapType type, StringView key, StringView value)
{
  return proxy_replaceHeaderMapValue(type, key.data(), key.size(), value.data(), value.size());
}

inline WasmResult
removeHeaderMapValue(HeaderMapType type, StringView key)
{
  return proxy_removeHeaderMapValue(type, key.data(), key.size());
}

inline WasmDataPtr
getHeaderMapPairs(HeaderMapType type)
{
  const char *ptr = nullptr;
  size_t size     = 0;
...
inline WasmResult
setHeaderMapPairs(HeaderMapType type, const HeaderStringPairs &pairs)
{
  const char *ptr = nullptr;
  size_t size     = 0;
...
inline WasmResult
setResponseTrailerPairs(const HeaderStringPairs &pairs)
{
  return setHeaderMapPairs(HeaderMapType::ResponseTrailers, pairs);
}
inline WasmResult
getResponseTrailerSize(size_t *size)
{
...
// Body
inline WasmDataPtr
getRequestBodyBufferBytes(size_t start, size_t length)
{
  const char *ptr = nullptr;
  size_t size     = 0;
...
inline WasmDataPtr
getResponseBodyBufferBytes(size_t start, size_t length)
{
  const char *ptr = nullptr;
  size_t size     = 0;
...
inline void
MakeHeaderStringPairsBuffer(const HeaderStringPairs &headers, void **buffer_ptr, size_t *size_ptr)
{
  if (headers.empty()) {
    *buffer_ptr = nullptr;
    *size_ptr   = 0;
...
inline uint32_t
makeHttpCall(StringView uri, const HeaderStringPairs &request_headers, StringView request_body,
             const HeaderStringPairs &request_trailers, uint32_t timeout_milliseconds)
{
  ...
  uint32_t result = proxy_httpCall(uri.data(), uri.size(), headers_ptr, headers_size, request_body.data(), request_body.size(),
                                   trailers_ptr, trailers_size, timeout_milliseconds);
  ::free(headers_ptr);
  ::free(trailers_ptr);
  return result;
}
...
inline WasmResult
defineMetric(MetricType type, StringView name, uint32_t *metric_id)
{
  return proxy_defineMetric(type, name.data(), name.size(), metric_id);
}

inline WasmResult
incrementMetric(uint32_t metric_id, int64_t offset)
{
  return proxy_incrementMetric(metric_id, offset);
}

inline WasmResult
recordMetric(uint32_t metric_id, uint64_t value)
{
  return proxy_recordMetric(metric_id, value);
}

inline WasmResult
getMetric(uint32_t metric_id, uint64_t *value)
{
...
struct MetricBase {
  ...
  MetricBase(MetricType t, const std::string &n, const std::vector<MetricTag> &ts) : type(t), name(n), tags(ts.begin(), ts.end()) {}
  MetricBase(MetricType t, const std::string &n, const std::vector<MetricTag> &ts, std::string fs, std::string vs)
...
struct Metric : public MetricBase {
  ...
  Metric(MetricType t, const std::string &n, const std::vector<MetricTag> &ts) : MetricBase(t, n, ts) {}
  Metric(MetricType t, const std::string &n, const std::vector<MetricTag> &ts, std::string field_separator,
...
inline std::string
MetricBase::prefixWithFields(const std::vector<std::string> &fields)
{
...
inline uint32_t
MetricBase::resolveWithFields(const std::vector<std::string> &fields)
{
...
inline void
MetricBase::partiallyResolveWithFields(const std::vector<std::string> &fields)
{
  if (fields.size() >= tags.size()) {
#if WASM_EXCEPTIONS
    throw ProxyException("metric fields.size() >= tags.size()");
...
inline uint32_t
MetricBase::resolveFullName(const std::string &n)
{
...
inline std::string
MetricBase::nameFromIdSlow(uint32_t id)
{
...
template <typename... Fields>
Metric
Metric::partiallyResolve(Fields... f)
{
  std::vector<std::string> fields{toString(f)...};
...
template <typename... Fields>
inline void
Metric::increment(int64_t offset, Fields... f)
{
  std::vector<std::string> fields{toString(f)...};
...
template <typename... Fields>
inline void
Metric::record(uint64_t value, Fields... f)
{
  std::vector<std::string> fields{toString(f)...};
  auto metric_id = resolveWithFields(fields);
...
template <typename T>
inline MetricTag
toMetricTag(const MetricTagDescriptor<T> &)
{
  return {};
}
...
template <>
inline MetricTag
toMetricTag(const MetricTagDescriptor<const char *> &d)
{
...
template <>
inline MetricTag
toMetricTag(const MetricTagDescriptor<std::string> &d)
{
  return {std::string(d.name), MetricTag::TagType::String};
}
...
template <>
inline MetricTag
toMetricTag(const MetricTagDescriptor<StringView> &d)
{
  return {std::string(d.name), MetricTag::TagType::String};
}
...
template <>
inline MetricTag
toMetricTag(const MetricTagDescriptor<int> &d)
{
...
template <>
inline MetricTag
toMetricTag(const MetricTagDescriptor<bool> &d)
{
...
struct SimpleCounter {
  ...
  void
  increment(int64_t offset)
  {
    recordMetric(metric_id, offset);
  }
  void
  record(int64_t offset)
  {
    increment(offset);
  }
  uint64_t
...
template <typename... Tags> struct Counter : public MetricBase {
  ...
  void
  increment(int64_t offset, Tags... tags)
  {
  ...
  void
  record(int64_t offset, Tags... tags)
  {
    increment(offset, tags...);
  }
...
private:
  Counter(const std::string &name, const std::vector<MetricTag> &tags) : MetricBase(MetricType::Counter, name, tags) {}
};

template <typename... Tags>
inline Counter<Tags...> *
Counter<Tags...>::New(StringView name, MetricTagDescriptor<Tags>... descriptors)
{
  return new Counter<Tags...>(std::string(name), std::vector<MetricTag>({toMetricTag(descriptors)...}));
}
...
template <typename... Tags> struct Gauge : public MetricBase {
  ...
  void
  record(int64_t offset, typename StringToStringView<Tags>::type... tags)
  {
    std::vector<std::string> fields{toString(tags)...};
    auto metric_id = resolveWithFields(fields);
    recordMetric(metric_id, offset);
...
private:
  Gauge(const std::string &name, const std::vector<MetricTag> &tags) : MetricBase(MetricType::Gauge, name, tags) {}
};

template <typename... Tags>
inline Gauge<Tags...> *
Gauge<Tags...>::New(StringView name, MetricTagDescriptor<Tags>... descriptors)
{
  return new Gauge<Tags...>(std::string(name), std::vector<MetricTag>({toMetricTag(descriptors)...}));
}
...
private:
  Histogram(const std::string &name, const std::vector<MetricTag> &tags) : MetricBase(MetricType::Histogram, name, tags) {}
};

template <typename... Tags>
inline Histogram<Tags...> *
Histogram<Tags...>::New(StringView name, MetricTagDescriptor<Tags>... descriptors)
{
  return new Histogram<Tags...>(std::string(name), std::vector<MetricTag>({toMetricTag(descriptors)...}));
}

inline bool
RootContext::httpCall(StringView uri, const HeaderStringPairs &request_headers, StringView request_body,
                      const HeaderStringPairs &request_trailers, uint32_t timeout_milliseconds, HttpCallCallback callback)
{
...
inline void
RootContext::onHttpCallResponse(uint32_t token, std::unique_ptr<WasmData> header_pairs, std::unique_ptr<WasmData> body,
                                std::unique_ptr<WasmData> trailer_pairs)
{
...
inline WasmResult
ContextBase::setEffectiveContext()
{
