...
inline WasmResult sendLocalResponse(uint32_t response_code, StringView response_code_details,
    StringView body, const HeaderStringPairs& additional_response_headers, uint32_t grpc_status) {
  ...
  return proxy_sendLocalResponse(response_code, response_code_details.data(),
      response_code_details.size(), body.data(), body.size(), ptr, size, grpc_status);
}
inline WasmResult sendLocalResponse(uint32_t response_code, StringView response_code_details,
                                    StringView body,
  ...
  exportPairs(additional_response_headers, &ptr, &size);
  return proxy_sendLocalResponse(response_code, response_code_details.data(),
                                 response_code_details.size(), body.data(), body.size(), ptr, size, 0);
}
