...
public:
  ...
  // Send Response
  virtual void sendLocalResponse(uint64_t response_code, std::string_view body_text, Pairs additional_headers) {}

  // Shared Data
  virtual WasmResult getSharedData(std::string_view key,
  ...
  virtual WasmResult resolveSharedQueue(std::string_view vm_id, std::string_view queue_name,
                                        uint32_t* token);
  ...
  virtual void addHeaderMapValue(HeaderMapType type, std::string_view key,
  ...
  virtual void replaceHeaderMapValue(HeaderMapType type, std::string_view key,
                                     std::string_view value);
  ...
  virtual uint32_t httpCall(std::string_view cluster, const Pairs& request_headers,
                            std::string_view request_body, const Pairs& request_trailers,
                            int timeout_millisconds);
  virtual void httpRespond(const Pairs& response_headers, std::string_view body,
...
public:
  ...
  ~Wasm() { if (mutex_) TSMutexDestroy(mutex_); }

  TSMutex mutex() { return mutex_; }
  ...
  Context* getRootContext(std::string_view root_id) { return root_contexts_[std::string(root_id)].get(); }
...
std::shared_ptr<Wasm>
createWasm(std::string_view vm_id, std::string_view vm, const std::string& code, std::string_view vm_configuration, std::string_view root_id);
...
std::shared_ptr<Wasm> getThreadLocalWasm(std::string_view vm_id, std::string_view root_id,
                                         std::string_view configuration);
...
inline std::string_view Context::root_id() const {
  if (root_context_id_) {
...
inline uint64_t Wasm::copyString(std::string_view s) {
  if (s.empty()) {
    return 0; // nullptr
  }
  uint64_t pointer;
...
inline bool Wasm::copyToPointerSize(std::string_view s, uint64_t ptr_ptr, uint64_t size_ptr) {
  uint64_t pointer = 0;
