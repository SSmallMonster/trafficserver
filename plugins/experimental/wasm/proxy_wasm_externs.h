...
// State accessors
extern "C" WasmResult proxy_getProperty(const char *path_ptr, size_t path_size, const char **value_ptr_ptr, size_t *value_size_ptr);
extern "C" WasmResult proxy_setProperty(const char *path_ptr, size_t path_size, const char *value_ptr, size_t value_size);

// Continue/Reply/Route
extern "C" WasmResult proxy_continueRequest();
extern "C" WasmResult proxy_continueResponse();
extern "C" WasmResult proxy_sendLocalResponse(uint32_t response_code, const char *response_code_details_ptr,
...
// SharedData
// Returns: Ok, NotFound
extern "C" WasmResult proxy_getSharedData(const char *key_ptr, size_t key_size, const char **value_ptr, size_t *value_size,
                                          uint32_t *cas);
//  If cas != 0 and cas != the current cas for 'key' return false, otherwise set the value and
//  return true.
// Returns: Ok, CasMismatch
extern "C" WasmResult proxy_setSharedData(const char *key_ptr, size_t key_size, const char *value_ptr, size_t value_size,
                                          uint32_t cas);
...
// Returns: Ok, NotFound
extern "C" WasmResult proxy_resolveSharedQueue(const char *vm_id, size_t vm_id_size, const char *queue_name_ptr,
...
extern "C" WasmResult proxy_addHeaderMapValue(HeaderMapType type, const char *key_ptr, size_t key_size, const char *value_ptr,
                                              size_t value_size);
extern "C" WasmResult proxy_getHeaderMapValue(HeaderMapType type, const char *key_ptr, size_t key_size, const char **value_ptr,
                                              size_t *value_size);
...
extern "C" WasmResult proxy_replaceHeaderMapValue(HeaderMapType type, const char *key_ptr, size_t key_size, const char *value_ptr,
                                                  size_t value_size);
...
// Body
extern "C" WasmResult proxy_getRequestBodyBufferBytes(uint32_t start, uint32_t length, const char **ptr, size_t *size);
extern "C" WasmResult proxy_getResponseBodyBufferBytes(uint32_t start, uint32_t length, const char **ptr, size_t *size);
...
extern "C" uint32_t proxy_httpCall(const char *uri_ptr, size_t uri_size, void *header_pairs_ptr, size_t header_pairs_size,
                                   const char *body_ptr, size_t body_size, void *trailer_pairs_ptr, size_t trailer_pairs_size,
