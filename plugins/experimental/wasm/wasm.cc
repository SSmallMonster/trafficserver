...
#define WASM_CONTEXT(_c) (ContextOrEffectiveContext(static_cast<Context *>((void)_c, current_context_)))
...
{
  inline Word
  wasmResultToWord(WasmResult r)
  {
    return Word(static_cast<uint64_t>(r));
  }

  inline uint32_t
  convertWordToUint32(Word w)
  {
    return static_cast<uint32_t>(w.u64_);
  }
  ...
  public:
    WasmResult
    get(std::string_view vm_id, const std::string_view key, std::pair<std::string, uint32_t> *result)
    {
    ...
    WasmResult
    set(std::string_view vm_id, std::string_view key, std::string_view value, uint32_t cas)
    {
    ...
    uint32_t
    registerQueue(std::string_view vm_id, std::string_view queue_name, uint32_t context_id, TSEventThread thread)
    {
    ...
    WasmResult
    dequeue(uint32_t token, std::string *data)
    {
    ...
    WasmResult
    enqueue(uint32_t token, std::string_view value)
    {
      auto it = queues.find(token);
      if (it == queues.end()) {
        return WasmResult::NotFound;
  ...
  private:
    ...
    std::map<std::string, std::unordered_map<std::string, std::pair<std::string, uint32_t>>> data;
  ...
  template <typename Pairs>
  void
  marshalPairs(const Pairs &result, char *buffer)
  {
    char *b                          = buffer;
  ...
  template <typename Pairs>
  bool
  getPairs(Context *context, const Pairs &result, uint64_t ptr_ptr, uint64_t size_ptr)
  {
    if (result.empty()) {
  ...
  void
  exportPairs(Context *context, const Pairs &pairs, uint64_t *ptr_ptr, uint64_t *size_ptr)
  {
    if (pairs.empty()) {
      *ptr_ptr  = 0;
      *size_ptr = 0;
  ...
  const uint8_t *
  decodeVarint(const uint8_t *pos, const uint8_t *end, uint32_t *out)
  {
    uint32_t ret = 0;
    int shift    = 0;
  ...
  Context *
  ContextOrEffectiveContext(Context *context)
  {
...
Word
setPropertyHandler(void *raw_context, Word key_ptr, Word key_size, Word value_ptr, Word value_size)
{
...
// Generic selector
Word
getPropertyHandler(void *raw_context, Word path_ptr, Word path_size, Word value_ptr_ptr, Word value_size_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
continueRequestHandler(void *raw_context)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
continueResponseHandler(void *raw_context)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
sendLocalResponseHandler(void *raw_context, Word response_code, Word response_code_details_ptr, Word response_code_details_size,
                         Word body_ptr, Word body_size, Word additional_response_header_pairs_ptr,
                         Word additional_response_header_pairs_size)
{
  auto context = WASM_CONTEXT(raw_context);
  auto details = context->wasmVm()->getMemory(response_code_details_ptr.u64_, response_code_details_size.u64_);
  auto body    = context->wasmVm()->getMemory(body_ptr.u64_, body_size.u64_);
  auto additional_response_header_pairs =
    context->wasmVm()->getMemory(additional_response_header_pairs_ptr.u64_, additional_response_header_pairs_size.u64_);
...
Word
setEffectiveContextHandler(void *raw_context, Word context_id)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
clearRouteCacheHandler(void *raw_context)
{
  (void)raw_context;
  // Noop.
  return wasmResultToWord(WasmResult::Ok);
...
// SharedData
Word
getSharedDataHandler(void *raw_context, Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr, Word cas_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
  auto key     = context->wasmVm()->getMemory(key_ptr.u64_, key_size.u64_);
  if (!key) {
...
Word
setSharedDataHandler(void *raw_context, Word key_ptr, Word key_size, Word value_ptr, Word value_size, Word cas)
{
  auto context = WASM_CONTEXT(raw_context);
  auto key     = context->wasmVm()->getMemory(key_ptr.u64_, key_size.u64_);
  auto value   = context->wasmVm()->getMemory(value_ptr.u64_, value_size.u64_);
...
Word
registerSharedQueueHandler(void *raw_context, Word queue_name_ptr, Word queue_name_size, Word token_ptr)
{
  auto context    = WASM_CONTEXT(raw_context);
...
Word
dequeueSharedQueueHandler(void *raw_context, Word token, Word data_ptr_ptr, Word data_size_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
  std::string data;
...
Word
resolveSharedQueueHandler(void *raw_context, Word vm_id_ptr, Word vm_id_size, Word queue_name_ptr, Word queue_name_size,
                          Word token_ptr)
{
  auto context    = WASM_CONTEXT(raw_context);
...
Word
enqueueSharedQueueHandler(void *raw_context, Word token, Word data_ptr, Word data_size)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
addHeaderMapValueHandler(void *raw_context, Word type, Word key_ptr, Word key_size, Word value_ptr, Word value_size)
{
...
Word
getHeaderMapValueHandler(void *raw_context, Word type, Word key_ptr, Word key_size, Word value_ptr_ptr, Word value_size_ptr)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
Word
replaceHeaderMapValueHandler(void *raw_context, Word type, Word key_ptr, Word key_size, Word value_ptr, Word value_size)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
Word
removeHeaderMapValueHandler(void *raw_context, Word type, Word key_ptr, Word key_size)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
Word
getHeaderMapPairsHandler(void *raw_context, Word type, Word ptr_ptr, Word size_ptr)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
Word
setHeaderMapPairsHandler(void *raw_context, Word type, Word ptr, Word size)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
Word
getHeaderMapSizeHandler(void *raw_context, Word type, Word result_ptr)
{
  if (type.u64_ > static_cast<uint64_t>(HeaderMapType::MAX)) {
    return wasmResultToWord(WasmResult::BadArgument);
  }
...
// Body Buffer
Word
getRequestBodyBufferBytesHandler(void *raw_context, Word start, Word length, Word ptr_ptr, Word size_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
getResponseBodyBufferBytesHandler(void *raw_context, Word start, Word length, Word ptr_ptr, Word size_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
httpCallHandler(void *raw_context, Word uri_ptr, Word uri_size, Word header_pairs_ptr, Word header_pairs_size, Word body_ptr,
                Word body_size, Word trailer_pairs_ptr, Word trailer_pairs_size, Word timeout_milliseconds)
{
  ...
  auto trailer_pairs = context->wasmVm()->getMemory(trailer_pairs_ptr.u64_, trailer_pairs_size.u64_);
...
Word
defineMetricHandler(void *raw_context, Word metric_type, Word name_ptr, Word name_size, Word metric_id_ptr)
{
  ...
  auto result        = context->defineMetric(static_cast<Context::MetricType>(metric_type.u64_), name.value(), &metric_id);
  if (result != WasmResult::Ok) {
    return wasmResultToWord(result);
  }
...
Word
incrementMetricHandler(void *raw_context, Word metric_id, int64_t offset)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
recordMetricHandler(void *raw_context, Word metric_id, uint64_t value)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
getMetricHandler(void *raw_context, Word metric_id, Word result_uint64_ptr)
{
  auto context   = WASM_CONTEXT(raw_context);
  uint64_t value = 0;
...
Word
_emscripten_get_heap_sizeHandler(void *raw_context)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
_emscripten_memcpy_bigHandler(void *raw_context, Word dst, Word src, Word size)
{
  auto context = WASM_CONTEXT(raw_context);
...
Word
_emscripten_resize_heapHandler(void *, Word)
{
  throw WasmException("emscripten emscripten_resize_heap");
}

Word
abortOnCannotGrowMemoryAbi00Handler(void *)
{
  throw WasmException("emscripten abortOnCannotGrowMemory");
}

Word
abortOnCannotGrowMemoryAbi02Handler(void *, Word)
{
  throw WasmException("emscripten abortOnCannotGrowMemory");
}

void
abortHandler(void *, Word)
{
...
void
_llvm_trapHandler(void *)
{
  throw WasmException("emscripten llvm_trap");
}

void
___assert_failHandler(void *, Word, Word, Word, Word)
{
  throw WasmException("emscripten assert_fail");
}

void
___cxa_throwHandler(void *, Word, Word, Word)
{
  throw WasmException("emscripten cxa_throw");
}

void
___cxa_pure_virtualHandler(void *)
{
  throw WasmException("emscripten cxa_pure_virtual");
}

Word
___call_mainHandler(void *, Word, Word)
{
  throw WasmException("emscripten call_main");
}

Word
___cxa_allocate_exceptionHandler(void *, Word)
{
  throw WasmException("emscripten cxa_allocate_exception");
}

Word
___cxa_uncaught_exceptionHandler(void *)
{
  throw WasmException("emscripten cxa_uncaught_exception");
}

Word
___cxa_uncaught_exceptionsHandler(void *)
{
  throw WasmException("emscripten cxa_uncaught_exceptions");
}

Word
___clock_gettimeHandler(void *, Word, Word)
{
  throw WasmException("emscripten clock_gettime");
}

void
___lockHandler(void *, Word)
{
  throw WasmException("emscripten lock");
}

void
___unlockHandler(void *, Word)
{
  throw WasmException("emscripten unlock");
}

Word
___syscall6Handler(void *, Word, Word)
{
  throw WasmException("emscripten syscall6");
}

Word
___syscall54Handler(void *, Word, Word)
{
  throw WasmException("emscripten syscall54");
}

Word
___syscall140Handler(void *, Word, Word)
{
...
Word
writevImpl(void *raw_context, Word fd, Word iovs, Word iovs_len, Word *nwritten_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
  ...
  for (size_t i = 0; i < iovs_len.u64_; i++) {
    auto memslice = context->wasmVm()->getMemory(iovs.u64_ + i * 2 * sizeof(uint32_t), 2 * sizeof(uint32_t));
    if (!memslice) {
...
Word
___syscall146Handler(void *raw_context, Word, Word syscall_args_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
  ...
  Word nwritten(0);
  auto result = writevImpl(raw_context, Word(syscall_args[0]), Word(syscall_args[1]), Word(syscall_args[2]), &nwritten);
...
// nwritten);
Word
___wasi_fd_writeHandler(void *raw_context, Word fd, Word iovs, Word iovs_len, Word nwritten_ptr)
{
  auto context = WASM_CONTEXT(raw_context);
...
void
___setErrNoHandler(void *, Word)
{
  throw WasmException("emscripten setErrNo");
}

Word
_pthread_equalHandler(void *, Word left, Word right)
{
  return left.u64_ == right.u64_;
}
// NB: pthread_mutex_destroy is required to return 0 by the protobuf libarary.
Word
_pthread_mutex_destroyHandler(void *, Word)
{
  return 0;
}
Word
_pthread_cond_waitHandler(void *, Word, Word)
{
  throw WasmException("emscripten pthread_cond_wait");
}
Word
_pthread_getspecificHandler(void *, Word)
{
  throw WasmException("emscripten pthread_getspecific");
}
Word
_pthread_key_createHandler(void *, Word, Word)
{
  throw WasmException("emscripten pthread_key_create");
}
Word
_pthread_onceHandler(void *, Word, Word)
{
  throw WasmException("emscripten pthread_once");
}
Word
_pthread_setspecificHandler(void *, Word, Word)
{
  throw WasmException("emscripten pthread_setspecific");
}
void
setTempRet0Handler(void *, Word)
{
  throw WasmException("emscripten setTempRet0");
}

double
globalMathLogHandler(void *, double f)
{
  return ::log(f);
}

Word
setTickPeriodMillisecondsHandler(void *raw_context, Word tick_period_milliseconds)
{
  return wasmResultToWord(WASM_CONTEXT(raw_context)->setTickPeriod(tick_period_milliseconds.u64_ * 1000 * 1000)); // -> nsec
}

Word
getCurrentTimeNanosecondsHandler(void *raw_context, Word result_uint64_ptr)
{
  auto context    = WASM_CONTEXT(raw_context);
...
Word
logHandler(void *raw_context, Word level, Word address, Word size)
{
  auto context = WASM_CONTEXT(raw_context);
...
WasmResult
Context::setTickPeriod(uint64_t tick_period)
{
...
WasmResult
Context::setSharedData(std::string_view key, std::string_view value, uint32_t cas)
{
...
uint32_t
Context::registerSharedQueue(std::string_view queue_name)
{
  // Get the id of the root context if this is a stream context because onQueueReady is on the root.
  return global_shared_data.registerQueue(wasm_->id(), queue_name, isRootContext() ? id_ : root_context_id_, TSEventThreadSelf());
}

WasmResult
Context::resolveSharedQueue(std::string_view vm_id, std::string_view queue_name, uint32_t *token_ptr)
{
...
WasmResult
Context::dequeueSharedQueue(uint32_t token, std::string *data)
{
  return global_shared_data.dequeue(token, data);
}

WasmResult
Context::enqueueSharedQueue(uint32_t token, std::string_view value)
{
...
HeaderMap
Context::getHeaderMap(HeaderMapType type)
{
  HeaderMap map;
  switch (type) {
...
void
Context::addHeaderMapValue(HeaderMapType type, std::string_view key, std::string_view value)
{
...
std::string_view
Context::getHeaderMapValue(HeaderMapType type, std::string_view key)
{
  auto map = getHeaderMap(type);
  if (map.bufp) {
...
Pairs
headerMapToPairs(HeaderMap map)
{
  if (!map.bufp) {
    return {};
  }
...
Pairs
Context::getHeaderMapPairs(HeaderMapType type)
{
  return headerMapToPairs(getHeaderMap(type));
}

void
Context::setHeaderMapPairs(HeaderMapType type, const Pairs &pairs)
{
  auto map = getHeaderMap(type);
  if (!map.bufp) {
    return;
...
void
Context::removeHeaderMapValue(HeaderMapType type, std::string_view key)
{
  auto map = getHeaderMap(type);
  if (!map.bufp) {
    return;
...
void
Context::replaceHeaderMapValue(HeaderMapType type, std::string_view key, std::string_view value)
{
  auto map = getHeaderMap(type);
  if (!map.bufp) {
    return;
...
uint32_t
Context::getHeaderMapSize(HeaderMapType type)
{
  auto map = getHeaderMap(type);
  if (!map.bufp)
    return 0;
...
std::string_view
Context::getRequestBodyBufferBytes(uint32_t start, uint32_t length)
{
  return "";
}

std::string_view
Context::getResponseBodyBufferBytes(uint32_t start, uint32_t length)
{
  return "";
}
...
// Async call via HTTP
uint32_t
Context::httpCall(std::string_view cluster, const Pairs &request_headers, std::string_view request_body,
                  const Pairs &request_trailers, int timeout_milliseconds)
{
  return 0;
}

void
Context::httpRespond(const Pairs &response_headers, std::string_view body, const Pairs &response_trailers)
{
...
void
Context::scriptLog(LogLevel level, std::string_view message)
{
  switch (level) {
  case LogLevel::trace:
...
//
void
Context::onStart(std::string_view root_id, std::string_view vm_configuration)
{
  if (wasm_->onStart_) {
...
bool
Context::validateConfiguration(std::string_view configuration)
{
...
bool
Context::onConfigure(std::string_view configuration)
{
...
FilterDataStatus
Context::onRequestBody(int body_buffer_length, bool end_of_stream)
{
  if (!wasm_->onRequestBody_) {
    return FilterDataStatus::Continue;
  }
  switch (wasm_->onRequestBody_(this, id_, static_cast<uint32_t>(body_buffer_length), static_cast<uint32_t>(end_of_stream)).u64_) {
  case 0:
    return FilterDataStatus::Continue;
  case 1:
...
FilterDataStatus
Context::onResponseBody(int body_buffer_length, bool end_of_stream)
{
  if (!wasm_->onResponseBody_) {
    return FilterDataStatus::Continue;
  }
  switch (wasm_->onResponseBody_(this, id_, static_cast<uint32_t>(body_buffer_length), static_cast<uint32_t>(end_of_stream)).u64_) {
  case 0:
    return FilterDataStatus::Continue;
  case 1:
    return FilterDataStatus::StopIterationAndBuffer;
...
void
Context::onHttpCallResponse(uint32_t token, const Pairs &response_headers, std::string_view response_body,
...
void
Context::onQueueReady(uint32_t token)
{
...
WasmResult
Context::defineMetric(MetricType type, std::string_view name, uint32_t *metric_id_ptr)
{
...
WasmResult
Context::getMetric(uint32_t metric_id, uint64_t *result_uint64_ptr)
{
  return WasmResult::NotFound;
}

Wasm::Wasm(std::string_view vm, std::string_view id, std::string_view vm_configuration) : vm_configuration_(vm_configuration)
{
...
#define _REGISTER_ABI(_fn, _abi)        \
  wasm_vm_->registerCallback(           \
    ...
    &ConvertFunctionWordToUint32<decltype(_fn##_abi##Handler), _fn##_abi##Handler>::convertFunctionWordToUint32)
...
#define _REGISTER_PROXY(_fn)                                         \
  wasm_vm_->registerCallback("envoy", "_proxy_" #_fn, &_fn##Handler, \
                             &ConvertFunctionWordToUint32<decltype(_fn##Handler), _fn##Handler>::convertFunctionWordToUint32);
  _REGISTER_PROXY(log);
...
void
Wasm::establishEnvironment()
{
  if (is_emscripten_) {
    wasm_vm_->setMemoryLayout(emscripten_stack_base_, emscripten_dynamic_base_, emscripten_dynamictop_ptr_);

    global_table_base_ = wasm_vm_->makeGlobal("env", "__table_base", Word(0));
    global_dynamictop_ = wasm_vm_->makeGlobal("env", "DYNAMICTOP_PTR", Word(emscripten_dynamictop_ptr_));

    wasm_vm_->makeModule("global");
    global_NaN_      = wasm_vm_->makeGlobal("global", "NaN", std::nan("0"));
    global_Infinity_ = wasm_vm_->makeGlobal("global", "Infinity", std::numeric_limits<double>::infinity());
  }
}
...
bool
Wasm::initialize(const std::string &code, std::string_view name, bool allow_precompiled)
{
  if (!wasm_vm_) {
    return false;
  }
...
bool
Wasm::configure(Context *root_context, std::string_view configuration)
{
  if (!onConfigure_) {
    return true;
  }
...
Context *
Wasm::start(std::string_view root_id, std::string_view vm_configuration)
{
...
void
Wasm::startForTesting(std::unique_ptr<Context> context)
{
  auto context_ptr = context.get();
  if (!context->wasm_) {
...
// ATS time is in ns
void
Wasm::setTickPeriod(uint32_t context_id, uint64_t new_tick_period)
{
  (void)context_id;
...
void
Wasm::tickHandler(uint32_t root_context_id)
{
  if (onTick_) {
...
void
Wasm::queueReady(uint32_t root_context_id, uint32_t token)
{
...
TSCont
Context::initialize(TSCont global_contp, TSHttpTxn txnp)
{
...
void
Context::onHttpCallSuccess(uint32_t token)
{
...
void
Context::onHttpCallFailure(uint32_t token)
{
  onHttpCallResponse(token, {}, "", {});
  http_request_.erase(token);
}
...
void
AsyncClientHandler::onSuccess()
{
  context->onHttpCallSuccess(token);
}

void
AsyncClientHandler::onFailure()
{
  context->onHttpCallFailure(token);
}

static std::shared_ptr<Wasm>
createWasmInternal(std::string_view vm_id, std::string_view vm, const std::string &code, std::string_view vm_configuration,
...
std::shared_ptr<Wasm>
createWasm(std::string_view vm_id, std::string_view vm, const std::string &code, std::string_view vm_configuration,
...
std::shared_ptr<Wasm>
createThreadLocalWasm(Wasm &base_wasm, std::string_view root_id, std::string_view configuration)
{
...
std::shared_ptr<Wasm>
getThreadLocalWasm(std::string_view vm_id, std::string_view id, std::string_view configuration)
{
...
std::shared_ptr<Wasm>
getThreadLocalWasmOrNull(std::string_view id)
{
